...
import net.mcreator.familymodtime.world.dimension.CandylandDimension;
import net.mcreator.familymodtime.FamilymodtimeModElements;

import java.util.Random;

@FamilymodtimeModElements.ModElement.Tag
public class PiosonBlock extends FamilymodtimeModElements.ModElement {
	@ObjectHolder("familymodtime:pioson")
	public static final FlowingFluidBlock block = null;
	@ObjectHolder("familymodtime:pioson_bucket")
	public static final Item bucket = null;
	private FlowingFluid flowing = null;
	private FlowingFluid still = null;
	private ForgeFlowingFluid.Properties fluidproperties = null;
	public PiosonBlock(FamilymodtimeModElements instance) {
		super(instance, 20);
	...
	public void initElements() {
		fluidproperties = new ForgeFlowingFluid.Properties(() -> still, () -> flowing,
				FluidAttributes.builder(new ResourceLocation("familymodtime:blocks/poison2"), new ResourceLocation("familymodtime:blocks/poison2"))
						.luminosity(0).density(1000).viscosity(1000)).bucket(() -> bucket).block(() -> block);
		still = (FlowingFluid) new ForgeFlowingFluid.Source(fluidproperties).setRegistryName("pioson");
		flowing = (FlowingFluid) new ForgeFlowingFluid.Flowing(fluidproperties).setRegistryName("pioson_flowing");
		elements.blocks.add(() -> new FlowingFluidBlock(still, Block.Properties.create(Material.LAVA)) {
		}.setRegistryName("pioson"));
		elements.items.add(() -> new BucketItem(still, new Item.Properties().containerItem(Items.BUCKET).maxStackSize(1).group(ItemGroup.MISC))
				.setRegistryName("pioson_bucket"));
	}
		...
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			biome.addFeature(GenerationStage.Decoration.LOCAL_MODIFICATIONS, new LakesFeature(BlockStateFeatureConfig::deserialize) {
				...
				public boolean place(IWorld world, ChunkGenerator generator, Random rand, BlockPos pos, BlockStateFeatureConfig config) {
					...
					if (dimensionType == CandylandDimension.type)
						dimensionCriteria = true;
